...
네이버 플레이스용 콘텐츠 생성·검사·등록 경계를 실험한 개인 Workflow 프로젝트

> 구현된 브라우저 등록 경로와 장기 상용 운영 성과는 구분합니다. 크론 설정·비용·순위 관련 내용은 설정 또는 탐색 예시이며, 실제 운영 성과로 주장하지 않습니다.

## 🎯 주요 기능

- 📈 **순위 이력 기록**: 수집된 순위 정보를 조회·분석하기 위한 기록 경로
- 🤖 **콘텐츠 생성**: 사진 분석과 문맥을 반영한 마케팅 문구 생성
- 📸 **새 소식 등록 경로**: 명시적 실행 옵션에서 Playwright로 등록을 시도하고 로그 기록
- 💬 **리뷰 답글 Workflow**: 리뷰 판단과 답글 생성, 애매한 사례의 수동 검토 경계
- ✅ **정책 검사·재생성**: 금지어·정책 검사와 최대 3회 재생성
- 🔐 **브라우저 로그인 경로**: 실제 계정 사용 전 별도 검증이 필요한 자동화 경로
...
```mermaid
graph LR
    A[선택적 크론 설정 예시] --> B[Auto Poster 시작]
...
- [docs/METRICS_ANALYSIS.md](docs/METRICS_ANALYSIS.md) - 과거 탐색용 수치 메모 (운영 성과 근거로 사용하지 않음)
